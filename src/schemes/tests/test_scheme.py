...
class SchemeViewSetTestCase(APITestCase):
    ...
    def test_create_scheme(self):
        data = {
            "name": "Give all unemployed with household kids 1000",
            "benefits": [{"amount": 1000, "apply_household": True}],
            "criterias": [
                {
                    "name": "All unemployed",
                    "ops": "=",
    ...
    def test_get_scheme(self):
        ...
        self.assertEqual(
            response.data[0]["name"], "Give all unemployed with household kids 1000"
        )

    def test_partial_update_scheme(self):
        data = {"name": "random"}
        response = self.client.patch(
            f"{self.base_url}/{self.scheme.uid}", data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["name"], data["name"])

    def test_eligible(self):
        response = self.client.get(f"{self.base_url}/eligible?applicant=", follow=True)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(
            response.data[0]["name"], "Give all unemployed with household kids 1000"
