...
class ApplicantHousehold(BaseModel):
    ...
    marital_status = models.CharField(
        choices=Applicant.MaritalStatus.choices,
        max_length=20,
        default=Applicant.MaritalStatus.SINGLE,
    )
    employment_status = models.CharField(
        choices=Applicant.EmploymentStatus.choices,
        max_length=50,
        default=Applicant.EmploymentStatus.UNEMPLOYED,
    )
    sex = models.CharField(
        choices=Applicant.Sex.choices, max_length=10, null=True, blank=True
    )
    applicant_id = models.ForeignKey(
        Applicant, related_name="households", on_delete=models.PROTECT
    )
